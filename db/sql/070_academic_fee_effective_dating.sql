...
-- Additive migration — applied after 068 via tools/scripts/apply-sql.sh.
--
...
-- (062 created the run/line/ledger tables; service previously used an in-memory Map)
-- ---------------------------------------------------------------------------
ALTER TABLE staff_payroll_runs
  ADD COLUMN IF NOT EXISTS csv_artifact TEXT;
ALTER TABLE staff_payroll_runs
  ADD COLUMN IF NOT EXISTS trial_balance_json JSONB;
ALTER TABLE staff_payroll_runs
  ADD COLUMN IF NOT EXISTS lines_json JSONB;

-- ---------------------------------------------------------------------------
-- staff_payroll_runs — durable export artifacts for active StaffHrService
-- ---------------------------------------------------------------------------
ALTER TABLE staff_payroll_runs
  ADD COLUMN IF NOT EXISTS csv_artifact TEXT,
  ADD COLUMN IF NOT EXISTS trial_balance_json JSONB,
