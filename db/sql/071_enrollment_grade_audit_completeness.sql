-- W1-DATA-14 — Enrollment history + grade-change audit completeness (DB-enforced).
--
-- Finding: history/audit rows were application-conventional (service inserts after
-- DML). Raw SQL or a forgotten app path could change enrollment status or grade
-- scores without an audit trail.
--
-- Posture after this file:
--   1. enrollment_history is written by triggers on enrollments INSERT / status UPDATE
--   2. grade_change_audit is written by triggers on grade_entries INSERT / material UPDATE
--   3. Both audit tables are append-only (UPDATE/DELETE blocked) + REVOKE from proctira_app
--   4. grade_change_audit.grade_entry_id FK → grade_entries(id) (additive, NOT VALID then VALIDATE)
--
-- Optional session enrichment (same transaction as the parent DML):
--   app.enrollment_history_reason, app.enrollment_history_effective_date
--   app.grade_change_action, app.grade_change_actor_id
-- App-level INSERT into the audit tables remains allowed (enrichment / back-compat);
-- triggers guarantee at least one row so completeness cannot be skipped.

CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- ---------------------------------------------------------------------------
-- 1. grade_change_audit: default id + required FK to grade_entries
-- ---------------------------------------------------------------------------
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'grade_change_audit_grade_entry_fk'
  ) THEN
    ALTER TABLE grade_change_audit
      ADD CONSTRAINT grade_change_audit_grade_entry_fk
...
-- Validate only when no orphans exist (fresh installs / cleaned DBs).
DO $$
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE 'W1-DATA-14: skipping VALIDATE on grade_change_audit_grade_entry_fk (orphan rows present)';
  ELSE
    ALTER TABLE grade_change_audit VALIDATE CONSTRAINT grade_change_audit_grade_entry_fk;
  END IF;
EXCEPTION
...
-- ---------------------------------------------------------------------------
-- 2. Append-only guards on audit/history tables
-- ---------------------------------------------------------------------------
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'proctira_app') THEN
    REVOKE UPDATE, DELETE ON enrollment_history FROM proctira_app;
    REVOKE TRUNCATE ON enrollment_history FROM proctira_app;
    REVOKE TRIGGER ON enrollment_history FROM proctira_app;

    REVOKE UPDATE, DELETE ON grade_change_audit FROM proctira_app;
    REVOKE TRUNCATE ON grade_change_audit FROM proctira_app;
    REVOKE TRIGGER ON grade_change_audit FROM proctira_app;
  END IF;
END $$;

-- ---------------------------------------------------------------------------
-- 3. Enrollment status → enrollment_history (cannot skip)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. Grade entry material changes → grade_change_audit (cannot skip)
-- ---------------------------------------------------------------------------
...
VALUES ('071_enrollment_grade_audit_completeness.sql')
