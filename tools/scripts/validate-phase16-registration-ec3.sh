#!/usr/bin/env bash
# Phase 16 EC3 validation — registration schema boundary.
set -euo pipefail
...
echo "==> SQL: registration schema + forbidden cross-schema FKs"
run_sql <<'SQL'
SELECT nspname FROM pg_namespace
 ...
 ORDER BY 1;
SELECT count(*) AS registration_applications FROM registration.registration_applications;
SELECT count(*) AS forbidden_registration_fks
FROM pg_constraint c
JOIN pg_class t ON t.oid = c.conrelid JOIN pg_namespace n ON n.oid = t.relnamespace
JOIN pg_class ft ON ft.oid = c.confrelid JOIN pg_namespace fn ON fn.oid = ft.relnamespace
WHERE c.contype = 'f'
  AND n.nspname = 'registration'
  AND fn.nspname IS DISTINCT FROM 'registration';
SQL
...
for path in /api/v1/students /api/v1/staff /api/v1/examinations; do
  code="$(curl -sS -o /tmp/p16.json -w '%{http_code}' \
    "${API_URL}${path}" -H "Authorization: Bearer ${access}" || true)"
  echo "${path} -> HTTP ${code}"
done
echo "Phase 16 EC3 validation finished"
