#!/usr/bin/env bash
# Phase 11 EC3 validation — health schema boundary.
set -euo pipefail
...
echo "==> SQL: health schema + forbidden cross-schema FKs"
run_sql <<'SQL'
SELECT nspname FROM pg_namespace
 ...
 ORDER BY 1;
SELECT count(*) AS health_measurements FROM health.health_measurements;
SELECT count(*) AS allergies FROM health.allergies;
SELECT count(*) AS health_conditions FROM health.health_conditions;
SELECT count(*) AS vaccinations FROM health.vaccinations;
SELECT count(*) AS forbidden_health_fks
FROM pg_constraint c
JOIN pg_class t ON t.oid = c.conrelid JOIN pg_namespace n ON n.oid = t.relnamespace
JOIN pg_class ft ON ft.oid = c.confrelid JOIN pg_namespace fn ON fn.oid = ft.relnamespace
WHERE c.contype = 'f'
  AND n.nspname = 'health'
  AND fn.nspname IS DISTINCT FROM 'health';
SQL
...
for path in /api/v1/students /api/v1/staff /api/v1/examinations; do
  code="$(curl -sS -o /tmp/p11.json -w '%{http_code}' \
    "${API_URL}${path}" -H "Authorization: Bearer ${access}" || true)"
  echo "${path} -> HTTP ${code}"
done
echo "Phase 11 EC3 validation finished"
