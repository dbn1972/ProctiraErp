#!/usr/bin/env bash
# Phase 10 EC3 validation — transport schema boundary.
set -euo pipefail
...
echo "==> SQL: transport schema + forbidden cross-schema FKs"
run_sql <<'SQL'
SELECT nspname FROM pg_namespace
 ...
 ORDER BY 1;
SELECT count(*) AS transport_routes FROM transport.transport_routes;
SELECT count(*) AS route_stops FROM transport.route_stops;
SELECT count(*) AS vehicles FROM transport.vehicles;
SELECT count(*) AS driver_assignments FROM transport.driver_assignments;
SELECT count(*) AS forbidden_transport_fks
FROM pg_constraint c
JOIN pg_class t ON t.oid = c.conrelid JOIN pg_namespace n ON n.oid = t.relnamespace
JOIN pg_class ft ON ft.oid = c.confrelid JOIN pg_namespace fn ON fn.oid = ft.relnamespace
WHERE c.contype = 'f'
  AND n.nspname = 'transport'
  AND fn.nspname IS DISTINCT FROM 'transport';
SQL
...
for path in /api/v1/students /api/v1/staff /api/v1/examinations; do
  code="$(curl -sS -o /tmp/p10.json -w '%{http_code}' \
    "${API_URL}${path}" -H "Authorization: Bearer ${access}" || true)"
  echo "${path} -> HTTP ${code}"
done
echo "Phase 10 EC3 validation finished"
