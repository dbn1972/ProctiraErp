#!/usr/bin/env bash
# Phase 9 EC3 validation — scholarship schema boundary.
set -euo pipefail
...
echo "==> SQL: scholarship schema + forbidden cross-schema FKs"
run_sql <<'SQL'
SELECT nspname FROM pg_namespace
 ...
 ORDER BY 1;
SELECT count(*) AS scholarship_programs FROM scholarship.scholarship_programs;
SELECT count(*) AS scholarship_applications FROM scholarship.scholarship_applications;
SELECT count(*) AS scholarship_disbursements FROM scholarship.scholarship_disbursements;
SELECT count(*) AS scholarship_compliance_records FROM scholarship.scholarship_compliance_records;
SELECT count(*) AS forbidden_scholarship_fks
FROM pg_constraint c
JOIN pg_class t ON t.oid = c.conrelid JOIN pg_namespace n ON n.oid = t.relnamespace
JOIN pg_class ft ON ft.oid = c.confrelid JOIN pg_namespace fn ON fn.oid = ft.relnamespace
WHERE c.contype = 'f'
  AND n.nspname = 'scholarship'
  AND fn.nspname IS DISTINCT FROM 'scholarship';
SQL
...
for path in /api/v1/students /api/v1/staff /api/v1/examinations; do
  code="$(curl -sS -o /tmp/p9.json -w '%{http_code}' \
    "${API_URL}${path}" -H "Authorization: Bearer ${access}" || true)"
  echo "${path} -> HTTP ${code}"
done
echo "Phase 9 EC3 validation finished"
