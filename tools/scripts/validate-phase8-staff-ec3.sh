#!/usr/bin/env bash
# Phase 8 EC3 validation — staff schema boundary.
set -euo pipefail
...
echo "==> SQL: staff schema + forbidden cross-schema FKs"
run_sql <<'SQL'
SELECT nspname FROM pg_namespace
 WHERE nspname IN (
   'platform','auth','institution','student','attendance','assessment','examination','staff','public'
 )
 ORDER BY 1;
SELECT count(*) AS staff FROM staff.staff;
SELECT count(*) AS staff_assignments FROM staff.staff_assignments;
SELECT count(*) AS forbidden_staff_fks
FROM pg_constraint c
JOIN pg_class t ON t.oid = c.conrelid JOIN pg_namespace n ON n.oid = t.relnamespace
JOIN pg_class ft ON ft.oid = c.confrelid JOIN pg_namespace fn ON fn.oid = ft.relnamespace
WHERE c.contype = 'f'
  AND n.nspname = 'staff'
  AND fn.nspname IS DISTINCT FROM 'staff';
SQL

echo "==> Auth + staff API smoke"
...
for path in /api/v1/staff /api/v1/students /api/v1/examinations; do
  code="$(curl -sS -o /tmp/p8.json -w '%{http_code}' \
    "${API_URL}${path}" -H "Authorization: Bearer ${access}" || true)"
  echo "${path} -> HTTP ${code}"
done
echo "Phase 8 EC3 validation finished"
