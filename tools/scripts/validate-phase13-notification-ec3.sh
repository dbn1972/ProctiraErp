#!/usr/bin/env bash
# Phase 13 EC3 validation — notification schema boundary.
set -euo pipefail
...
echo "==> SQL: notification schema + forbidden cross-schema FKs"
run_sql <<'SQL'
SELECT nspname FROM pg_namespace
 ...
 ORDER BY 1;
SELECT count(*) AS notifications FROM notification.notifications;
SELECT count(*) AS notification_rules FROM notification.notification_rules;
SELECT count(*) AS notification_templates FROM notification.notification_templates;
SELECT count(*) AS forbidden_notification_fks
FROM pg_constraint c
JOIN pg_class t ON t.oid = c.conrelid JOIN pg_namespace n ON n.oid = t.relnamespace
JOIN pg_class ft ON ft.oid = c.confrelid JOIN pg_namespace fn ON fn.oid = ft.relnamespace
WHERE c.contype = 'f'
  AND n.nspname = 'notification'
  AND fn.nspname IS DISTINCT FROM 'notification';
SQL
...
for path in /api/v1/students /api/v1/staff /api/v1/examinations; do
  code="$(curl -sS -o /tmp/p13.json -w '%{http_code}' \
    "${API_URL}${path}" -H "Authorization: Bearer ${access}" || true)"
  echo "${path} -> HTTP ${code}"
done
echo "Phase 13 EC3 validation finished"
