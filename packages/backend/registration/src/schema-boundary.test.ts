/**
 * Phase 16 registration schema boundary checks (charter §19).
 * Registration-owned models live in schema "registration" with bare tenant_id.
 */
...
describe('Phase 16 registration schema boundaries', () => {
  const schemaText = readFileSync(schemaPath, 'utf8');

  it('declares registration among service-owned schemas', () => {
    expect(schemaText).toMatch(SCHEMAS_RE);
  });

  it('places registration-owned models in @@schema("registration")', () => {
    for (const model of MODELS) {
      expect(modelBody(schemaText, model)).toContain('@@schema("registration")');
    }
  });

  it('keeps registration tenant ids as bare UUIDs (no Tenant @relation)', () => {
    for (const model of MODELS) {
