/**
 * Phase 13 notification schema boundary checks (charter §19).
 * Notification-owned models live in schema "notification" with bare tenant_id.
 */
...
const MODELS = ['Notification', 'NotificationRule', 'NotificationTemplate'] as const;
...
describe('Phase 13 notification schema boundaries', () => {
  const schemaText = readFileSync(schemaPath, 'utf8');

  it('declares notification among service-owned schemas', () => {
    expect(schemaText).toMatch(SCHEMAS_RE);
  });

  it('places notification-owned models in @@schema("notification")', () => {
    for (const model of MODELS) {
      expect(modelBody(schemaText, model)).toContain('@@schema("notification")');
    }
  });

  it('keeps notification tenant ids as bare UUIDs (no Tenant @relation)', () => {
    for (const model of MODELS) {
