/**
 * Phase 14 report schema boundary checks (charter §19).
 * Report-owned models live in schema "report" with bare tenant_id.
 */
...
const MODELS = ['ReportJob', 'ReportTemplate', 'ScheduledReport'] as const;
...
describe('Phase 14 report schema boundaries', () => {
  const schemaText = readFileSync(schemaPath, 'utf8');

  it('declares report among service-owned schemas', () => {
    expect(schemaText).toMatch(SCHEMAS_RE);
  });

  it('places report-owned models in @@schema("report")', () => {
    for (const model of MODELS) {
      expect(modelBody(schemaText, model)).toContain('@@schema("report")');
    }
  });

  it('keeps report tenant ids as bare UUIDs (no Tenant @relation)', () => {
    for (const model of MODELS) {
