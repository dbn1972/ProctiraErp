/**
 * Phase 12 workflow schema boundary checks (charter §19).
 * Workflow-owned models live in schema "workflow" with bare tenant_id.
 */
...
const MODELS = ['WorkflowDefinition', 'WorkflowInstance', 'TransitionAudit', 'Case'] as const;
...
describe('Phase 12 workflow schema boundaries', () => {
  const schemaText = readFileSync(schemaPath, 'utf8');

  it('declares workflow among service-owned schemas', () => {
    expect(schemaText).toMatch(SCHEMAS_RE);
  });

  it('places workflow-owned models in @@schema("workflow")', () => {
    for (const model of MODELS) {
      expect(modelBody(schemaText, model)).toContain('@@schema("workflow")');
    }
  });

  it('keeps workflow tenant ids as bare UUIDs (no Tenant @relation)', () => {
    for (const model of MODELS) {
