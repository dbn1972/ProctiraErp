/**
 * User invite request/response types (admin invite flow).
 */
...
/**
 * Lightweight invite payload validation (auth package avoids TypeBox).
 */
export function validateInviteUserInput(
  ...
  if (raw['displayName'] !== undefined && (!displayName || displayName.length === 0)) {
    errors.push({ field: 'displayName', rule: 'minLength', message: 'displayName cannot be empty' });
  }
