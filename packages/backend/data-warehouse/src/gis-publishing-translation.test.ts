...
describe('GIS Service (Requirement 15.3)', () => {
  ...
  beforeEach(async () => {
    ...
    const area = await warehouseService.createArea(tenantId, warehouseId, {
      name: 'India', areaId: 'IND', gid: 'AREA_IND', level: 0,
    });
    areaId = area.id;
  });
  ...
  describe('GeoJSON Layer Management', () => {
    ...
    it('should create a GeoJSON layer from base64-encoded data', async () => {
      const geojson = JSON.stringify({
        type: 'FeatureCollection',
        features: [
          { type: 'Feature', geometry: { type: 'Point', coordinates: [77.5, 12.9] }, properties: { name: 'Bangalore' } },
          { type: 'Feature', geometry: { type: 'Point', coordinates: [72.8, 19.0] }, properties: { name: 'Mumbai' } },
        ],
      });
  ...
  describe('Layer CRUD Operations', () => {
    it('should get a layer by ID', async () => {
      ...
      const created = await gisService.createLayer(tenantId, warehouseId, {
        areaId, name: 'Test Layer', layerType: 'geojson', data: geojson,
      });
    ...
    it('should update a layer', async () => {
      const created = await gisService.createLayer(tenantId, warehouseId, {
        areaId, name: 'Old Name', layerType: 'geojson',
      });
    ...
    it('should deactivate a layer', async () => {
      const created = await gisService.createLayer(tenantId, warehouseId, {
        areaId, name: 'Active Layer', layerType: 'geojson',
      });
    ...
    it('should delete a layer', async () => {
      const created = await gisService.createLayer(tenantId, warehouseId, {
        areaId, name: 'To Delete', layerType: 'geojson',
      });

      await gisService.deleteLayer(tenantId, warehouseId, created.id);
      await expect(
        gisService.getLayer(tenantId, warehouseId, created.id),
      ).rejects.toThrow(NotFoundError);
    });

    it('should list layers with pagination', async () => {
      await gisService.createLayer(tenantId, warehouseId, { areaId, name: 'Layer 1', layerType: 'geojson' });
      await gisService.createLayer(tenantId, warehouseId, { areaId, name: 'Layer 2', layerType: 'geojson' });
      await gisService.createLayer(tenantId, warehouseId, { areaId, name: 'Layer 3', layerType: 'geojson' });
    ...
    it('should filter layers by area', async () => {
      const area2 = await warehouseService.createArea(tenantId, warehouseId, {
        name: 'Karnataka', areaId: 'KA', gid: 'AREA_KA', level: 1, parentId: areaId,
      });

      await gisService.createLayer(tenantId, warehouseId, { areaId, name: 'India Layer', layerType: 'geojson' });
      await gisService.createLayer(tenantId, warehouseId, { areaId: area2.id, name: 'KA Layer', layerType: 'geojson' });
  ...
  describe('Area Hierarchy Linking', () => {
    it('should get layers by area hierarchy (parent and children)', async () => {
      const childArea = await warehouseService.createArea(tenantId, warehouseId, {
        name: 'Maharashtra', areaId: 'MH', gid: 'AREA_MH', level: 1, parentId: areaId,
      });

      await gisService.createLayer(tenantId, warehouseId, { areaId, name: 'India Boundary', layerType: 'geojson' });
      await gisService.createLayer(tenantId, warehouseId, { areaId: childArea.id, name: 'MH Boundary', layerType: 'geojson' });
    ...
    it('should throw NotFoundError for non-existent area in hierarchy query', async () => {
      await expect(
        gisService.getLayersByAreaHierarchy(tenantId, warehouseId, '00000000-0000-4000-8000-000000000000'),
      ).rejects.toThrow(NotFoundError);
    });
  });
});
...
describe('Publishing Service (Requirement 15.4)', () => {
  ...
  beforeEach(async () => {
    ...
    await warehouseService.createSubgroup(tenantId, warehouseId, { name: 'Total', gid: 'SG_TOTAL' });
    await warehouseService.createTimePeriod(tenantId, warehouseId, { timePeriod: '2023' });
    await warehouseService.createArea(tenantId, warehouseId, { name: 'India', areaId: 'IND', gid: 'AREA_IND', level: 0 });
  ...
  describe('Publish with GIS Layers', () => {
    it('should include GIS layers when requested', async () => {
      ...
      const geojson = JSON.stringify({
        type: 'FeatureCollection',
        features: [{ type: 'Feature', geometry: { type: 'Point', coordinates: [77.5, 12.9] }, properties: {} }],
      });
  ...
  describe('Publish History', () => {
    ...
    it('should filter publish history by target', async () => {
      ...
      const history = await publishingService.getPublishHistory(tenantId, warehouseId, { target: 'mobile' });
  ...
  describe('Error Handling', () => {
    it('should throw NotFoundError for non-existent warehouse', async () => {
      await expect(
        publishingService.publishData(tenantId, '00000000-0000-4000-8000-000000000000', { target: 'api' }),
      ).rejects.toThrow(NotFoundError);
    });
  });
});
...
describe('Translation Service (Requirement 15.5)', () => {
  ...
  beforeEach(async () => {
    ...
    const wh = await warehouseService.createWarehouse(tenantId, { name: 'Translation Test DW', defaultLanguage: 'en' });
    warehouseId = wh.id;

    const indicator = await warehouseService.createIndicator(tenantId, warehouseId, { name: 'Net Enrollment Rate', gid: 'NER_001' });
    indicatorId = indicator.id;

    const unit = await warehouseService.createUnit(tenantId, warehouseId, { name: 'Percent', gid: 'PCT' });
    unitId = unit.id;

    const area = await warehouseService.createArea(tenantId, warehouseId, { name: 'India', areaId: 'IND', gid: 'AREA_IND', level: 0 });
    areaId = area.id;
  });
  ...
  describe('Single Translation', () => {
    ...
    it('should update an existing translation (upsert)', async () => {
      await translationService.setTranslation(tenantId, warehouseId, {
        entityType: 'indicator', entityId: indicatorId, language: 'fr', field: 'name', value: 'Old Value',
      });

      const updated = await translationService.setTranslation(tenantId, warehouseId, {
        entityType: 'indicator', entityId: indicatorId, language: 'fr', field: 'name', value: 'New Value',
      });
    ...
    it('should create translations for different entity types', async () => {
      await translationService.setTranslation(tenantId, warehouseId, {
        entityType: 'unit', entityId: unitId, language: 'fr', field: 'name', value: 'Pourcentage',
      });
      await translationService.setTranslation(tenantId, warehouseId, {
        entityType: 'area', entityId: areaId, language: 'fr', field: 'name', value: 'Inde',
      });

      const unitTranslations = await translationService.getEntityTranslations(tenantId, warehouseId, 'unit', unitId);
      expect(unitTranslations.length).toBe(1);
      expect(unitTranslations[0]!.value).toBe('Pourcentage');

      const areaTranslations = await translationService.getEntityTranslations(tenantId, warehouseId, 'area', areaId);
    ...
    it('should reject translation for non-existent warehouse', async () => {
      await expect(
        translationService.setTranslation(tenantId, '00000000-0000-4000-8000-000000000000', {
          entityType: 'indicator', entityId: indicatorId, language: 'fr', field: 'name', value: 'Test',
        }),
      ).rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe('Batch Translations', () => {
    it('should set multiple translations in batch', async () => {
      const result = await translationService.setTranslationsBatch(tenantId, warehouseId, {
        translations: [
          { entityType: 'indicator', entityId: indicatorId, language: 'fr', field: 'name', value: 'Taux net' },
          { entityType: 'unit', entityId: unitId, language: 'fr', field: 'name', value: 'Pourcentage' },
          { entityType: 'area', entityId: areaId, language: 'fr', field: 'name', value: 'Inde' },
        ],
      });
    ...
    it('should report errors for invalid entities in batch', async () => {
      const result = await translationService.setTranslationsBatch(tenantId, warehouseId, {
        translations: [
          { entityType: 'indicator', entityId: indicatorId, language: 'fr', field: 'name', value: 'Valid' },
          { entityType: 'indicator', entityId: '00000000-0000-4000-8000-000000000000', language: 'fr', field: 'name', value: 'Invalid' },
        ],
      });
  ...
  describe('Translation Fallback', () => {
    it('should return translated value for requested language', async () => {
      await translationService.setTranslation(tenantId, warehouseId, {
        entityType: 'indicator', entityId: indicatorId, language: 'fr', field: 'name', value: 'Taux net de scolarisation',
      });

      const value = await translationService.getTranslatedValue(
        tenantId, warehouseId, 'indicator', indicatorId, 'name', 'fr', 'Net Enrollment Rate',
      );
    ...
    it('should fallback to default language when requested language not available', async () => {
      await translationService.setTranslation(tenantId, warehouseId, {
        entityType: 'indicator', entityId: indicatorId, language: 'en', field: 'name', value: 'Net Enrollment Rate (EN)',
      });

      const value = await translationService.getTranslatedValue(
        tenantId, warehouseId, 'indicator', indicatorId, 'name', 'ar', 'Net Enrollment Rate',
      );
    ...
    it('should return default value when no translation exists', async () => {
      const value = await translationService.getTranslatedValue(
        tenantId, warehouseId, 'indicator', indicatorId, 'name', 'zh', 'Net Enrollment Rate',
      );
  ...
  describe('Translation Export', () => {
    beforeEach(async () => {
      ...
      await translationService.setTranslation(tenantId, warehouseId, {
        entityType: 'indicator', entityId: indicatorId, language: 'ar', field: 'name', value: 'معدل الالتحاق الصافي',
      });
    });

    it('should export translations as JSON', async () => {
      const result = await translationService.exportTranslations(tenantId, warehouseId, { format: 'json' });
    ...
    it('should export translations as CSV', async () => {
      const result = await translationService.exportTranslations(tenantId, warehouseId, { format: 'csv' });
    ...
    it('should filter export by language', async () => {
      const result = await translationService.exportTranslations(tenantId, warehouseId, { format: 'json', language: 'fr' });
    ...
    it('should filter export by entity type', async () => {
      const result = await translationService.exportTranslations(tenantId, warehouseId, { format: 'json', entityType: 'indicator' });
  ...
  describe('Translation Import', () => {
    it('should import translations from JSON', async () => {
      const jsonContent = JSON.stringify([
        { entityType: 'indicator', entityId: indicatorId, language: 'es', field: 'name', value: 'Tasa neta de matriculación' },
        { entityType: 'unit', entityId: unitId, language: 'es', field: 'name', value: 'Porcentaje' },
      ]);
...
unit,${unitId},es,name,Porcentaje`;
    ...
    it('should import translations from base64-encoded content', async () => {
      const jsonContent = JSON.stringify([
        { entityType: 'indicator', entityId: indicatorId, language: 'es', field: 'name', value: 'Tasa neta' },
      ]);
    ...
    it('should track updated translations during import', async () => {
      ...
      await translationService.setTranslation(tenantId, warehouseId, {
        entityType: 'indicator', entityId: indicatorId, language: 'es', field: 'name', value: 'Old Value',
      });

      // Import with updated value
      const jsonContent = JSON.stringify([
        { entityType: 'indicator', entityId: indicatorId, language: 'es', field: 'name', value: 'New Value' },
      ]);
    ...
    it('should report errors for invalid entity types in import', async () => {
      const jsonContent = JSON.stringify([
        { entityType: 'invalid_type', entityId: indicatorId, language: 'es', field: 'name', value: 'Test' },
      ]);
  ...
  describe('List Translations', () => {
    beforeEach(async () => {
      ...
      await translationService.setTranslation(tenantId, warehouseId, {
        entityType: 'indicator', entityId: indicatorId, language: 'ar', field: 'name', value: 'معدل',
      });
      await translationService.setTranslation(tenantId, warehouseId, {
        entityType: 'unit', entityId: unitId, language: 'fr', field: 'name', value: 'Pourcentage',
      });
    ...
    it('should filter by language', async () => {
      const result = await translationService.listTranslations(tenantId, warehouseId, { language: 'fr' });
      expect(result.total).toBe(2);
    });

    it('should filter by entity type', async () => {
      const result = await translationService.listTranslations(tenantId, warehouseId, { entityType: 'indicator' });
      expect(result.total).toBe(2);
    });

    it('should filter by entity ID', async () => {
      const result = await translationService.listTranslations(tenantId, warehouseId, { entityId: indicatorId });
      expect(result.total).toBe(2);
    });
  });
});
