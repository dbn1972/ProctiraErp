...
export interface PipelineRepository {
  ...
  list(tenantId: string, filter: PipelineListFilter, page: number, pageSize: number): Promise<PipelineListResult>;
  ...
  listExecutions(pipelineId: string, tenantId: string, page: number, pageSize: number): Promise<{ data: PipelineExecution[]; total: number }>;
}
