...
export const CreateMeasurementSchema = Type.Object({
  ...
  date: Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Measurement date (YYYY-MM-DD)' }),
  ...
  bloodPressureSystolic: Type.Optional(Type.Number({ minimum: 0, description: 'Systolic blood pressure (mmHg)' })),
  bloodPressureDiastolic: Type.Optional(Type.Number({ minimum: 0, description: 'Diastolic blood pressure (mmHg)' })),
  heartRate: Type.Optional(Type.Number({ minimum: 0, description: 'Heart rate (bpm)' })),
  visionLeft: Type.Optional(Type.String({ maxLength: 20, description: 'Left eye vision (e.g., 20/20)' })),
  visionRight: Type.Optional(Type.String({ maxLength: 20, description: 'Right eye vision (e.g., 20/20)' })),
...
export const UpdateMeasurementSchema = Type.Object({
  date: Type.Optional(Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Measurement date' })),
  height: Type.Optional(Type.Number({ minimum: 0, description: 'Height in cm' })),
  weight: Type.Optional(Type.Number({ minimum: 0, description: 'Weight in kg' })),
  bmi: Type.Optional(Type.Number({ minimum: 0, description: 'Body Mass Index' })),
  bloodPressureSystolic: Type.Optional(Type.Number({ minimum: 0, description: 'Systolic blood pressure' })),
  bloodPressureDiastolic: Type.Optional(Type.Number({ minimum: 0, description: 'Diastolic blood pressure' })),
...
export const CreateAllergySchema = Type.Object({
  studentId: Type.String({ description: 'Student UUID' }),
  allergyType: Type.String({ minLength: 1, maxLength: 100, description: 'Type of allergy (e.g., food, drug, environmental)' }),
  ...
  treatment: Type.Optional(Type.String({ maxLength: 500, description: 'Treatment/management plan' })),
  diagnosedDate: Type.Optional(Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Date diagnosed' })),
});
...
export const UpdateAllergySchema = Type.Object({
  ...
  severity: Type.Optional(Type.String({ enum: ['mild', 'moderate', 'severe', 'life-threatening'] })),
...
export const CreateConditionSchema = Type.Object({
  studentId: Type.String({ description: 'Student UUID' }),
  conditionName: Type.String({ minLength: 1, maxLength: 200, description: 'Name of the condition' }),
  conditionType: Type.String({ minLength: 1, maxLength: 100, description: 'Type/category of condition' }),
  diagnosedDate: Type.Optional(Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Date diagnosed' })),
...
export const CreateVaccinationSchema = Type.Object({
  ...
  dateAdministered: Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Date administered' }),
  administeredBy: Type.Optional(Type.String({ maxLength: 200, description: 'Administered by (name/facility)' })),
  batchNumber: Type.Optional(Type.String({ maxLength: 100, description: 'Vaccine batch number' })),
  nextDueDate: Type.Optional(Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Next dose due date' })),
...
export const CreateInsuranceSchema = Type.Object({
  ...
  endDate: Type.Optional(Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Coverage end date' })),
  notes: Type.Optional(Type.String({ maxLength: 500, description: 'Additional notes' })),
});
...
export const CreateSpecialNeedsAssessmentSchema = Type.Object({
  studentId: Type.String({ description: 'Student UUID' }),
  assessmentDate: Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Assessment date' }),
...
export const CreateDiagnosisSchema = Type.Object({
  ...
  category: Type.String({ minLength: 1, maxLength: 100, description: 'Diagnosis category (e.g., learning, physical, sensory)' }),
...
export const CreateReferralSchema = Type.Object({
  ...
  referredTo: Type.String({ minLength: 1, maxLength: 200, description: 'Referred to (specialist/facility)' }),
  reason: Type.String({ minLength: 1, maxLength: 500, description: 'Reason for referral' }),
  status: Type.String({ enum: ['pending', 'scheduled', 'completed', 'cancelled'], description: 'Referral status' }),
  appointmentDate: Type.Optional(Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Appointment date' })),
...
export const CreateAccommodationPlanSchema = Type.Object({
  ...
  endDate: Type.Optional(Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Plan end date' })),
  accommodations: Type.Array(Type.Object({
    type: Type.String({ minLength: 1, maxLength: 100, description: 'Accommodation type' }),
    description: Type.String({ minLength: 1, maxLength: 500, description: 'Accommodation description' }),
  }), { minItems: 1, description: 'List of accommodations' }),
  reviewDate: Type.Optional(Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Next review date' })),
  status: Type.String({ enum: ['active', 'under-review', 'expired', 'cancelled'], description: 'Plan status' }),
  notes: Type.Optional(Type.String({ maxLength: 1000, description: 'Additional notes' })),
});
...
export const UpdateAccommodationPlanSchema = Type.Object({
  ...
  startDate: Type.Optional(Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$' })),
  endDate: Type.Optional(Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$' })),
  accommodations: Type.Optional(Type.Array(Type.Object({
...
export const CreateCounsellingSessionSchema = Type.Object({
  ...
  sessionType: Type.String({ enum: ['individual', 'group', 'family', 'crisis'], description: 'Session type' }),
  ...
  followUpDate: Type.Optional(Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Follow-up date' })),
  status: Type.String({ enum: ['scheduled', 'completed', 'cancelled', 'no-show'], description: 'Session status' }),
});
...
export const CreateScreeningProgramSchema = Type.Object({
  ...
  scheduledDate: Type.Optional(Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Scheduled date' })),
  status: Type.String({ enum: ['planned', 'in-progress', 'completed', 'cancelled'], description: 'Program status' }),
});
...
export const UpdateScreeningProgramSchema = Type.Object({
  ...
  assessmentTypes: Type.Optional(Type.Array(Type.String({ minLength: 1, maxLength: 100 }), { minItems: 1 })),
  scheduledDate: Type.Optional(Type.String({ pattern: '^\\d{4}-\\d{2}-\\d{2}$' })),
  status: Type.Optional(Type.String({ enum: ['planned', 'in-progress', 'completed', 'cancelled'] })),
});
