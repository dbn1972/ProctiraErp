...
describe('HealthService', () => {
  ...
  describe('hasHealthAccess', () => {
    ...
    it('grants access to counsellor role', () => {
      const ctx: HealthAccessContext = { userId: 'u1', roles: ['counsellor'], guardianOfStudentIds: [] };
      expect(hasHealthAccess(ctx, 'student-001')).toBe(true);
    });

    it('grants access to system_admin role', () => {
      const ctx: HealthAccessContext = { userId: 'u1', roles: ['system_admin'], guardianOfStudentIds: [] };
      expect(hasHealthAccess(ctx, 'student-001')).toBe(true);
    });
  });
  ...
  describe('Measurements', () => {
    it('creates a measurement for authorized user', async () => {
      const result = await service.createMeasurement(tenantId, {
    ...
    it('denies measurement creation for unauthorized user', async () => {
      await expect(
        service.createMeasurement(tenantId, {
          studentId: 'student-001',
          date: '2024-03-15',
          height: 165,
        }, unauthorizedContext),
    ...
    it('lists measurements by student', async () => {
      await service.createMeasurement(tenantId, { studentId: 'student-001', date: '2024-01-01', height: 160 }, healthOfficerContext);
      await service.createMeasurement(tenantId, { studentId: 'student-001', date: '2024-06-01', height: 165 }, healthOfficerContext);

      const result = await service.listMeasurements(tenantId, 'student-001', { page: 1, pageSize: 10 }, healthOfficerContext);
      expect(result.data).toHaveLength(2);
      expect(result.meta.totalItems).toBe(2);
    });

    it('updates a measurement', async () => {
      const created = await service.createMeasurement(tenantId, { studentId: 'student-001', date: '2024-03-15', height: 165 }, healthOfficerContext);
      const updated = await service.updateMeasurement(tenantId, created.id, { height: 167 }, healthOfficerContext);
    ...
    it('deletes a measurement', async () => {
      ...
      await expect(service.getMeasurement(tenantId, created.id, healthOfficerContext)).rejects.toThrow(NotFoundError);
    });
  });

  describe('Allergies', () => {
    it('creates an allergy record', async () => {
      const result = await service.createAllergy(tenantId, {
        studentId: 'student-001',
        allergyType: 'food',
    ...
    it('denies allergy creation for unauthorized user', async () => {
      await expect(
        service.createAllergy(tenantId, {
          studentId: 'student-001',
          allergyType: 'food',
          description: 'Peanut allergy',
          severity: 'severe',
    ...
    it('updates an allergy record', async () => {
      const created = await service.createAllergy(tenantId, {
        studentId: 'student-001',
        allergyType: 'food',
        description: 'Peanut allergy',
        severity: 'moderate',
      ...
      const updated = await service.updateAllergy(tenantId, created.id, { severity: 'severe' }, healthOfficerContext);
  ...
  describe('Conditions', () => {
    it('creates a health condition', async () => {
      const result = await service.createCondition(tenantId, {
        studentId: 'student-001',
    ...
    it('updates a condition status', async () => {
      const created = await service.createCondition(tenantId, {
        studentId: 'student-001',
        conditionName: 'Asthma',
        conditionType: 'respiratory',
        status: 'active',
      ...
      const updated = await service.updateCondition(tenantId, created.id, { status: 'resolved' }, healthOfficerContext);
  ...
  describe('Vaccinations', () => {
    it('creates a vaccination record', async () => {
      const result = await service.createVaccination(tenantId, {
        studentId: 'student-001',
        vaccineName: 'MMR',
        doseNumber: 1,
    ...
    it('lists vaccinations by student', async () => {
      await service.createVaccination(tenantId, { studentId: 'student-001', vaccineName: 'MMR', doseNumber: 1, dateAdministered: '2024-01-15' }, healthOfficerContext);
      await service.createVaccination(tenantId, { studentId: 'student-001', vaccineName: 'Polio', doseNumber: 1, dateAdministered: '2024-02-15' }, healthOfficerContext);

      const result = await service.listVaccinations(tenantId, 'student-001', { page: 1, pageSize: 10 }, healthOfficerContext);
      expect(result.data).toHaveLength(2);
    });
  });
  ...
  describe('Insurance', () => {
    it('creates an insurance record', async () => {
      const result = await service.createInsurance(tenantId, {
        studentId: 'student-001',
    ...
    it('updates insurance details', async () => {
      const created = await service.createInsurance(tenantId, {
        studentId: 'student-001',
        provider: 'HealthCare Inc.',
        policyNumber: 'POL-12345',
        coverageType: 'basic',
      ...
      const updated = await service.updateInsurance(tenantId, created.id, { coverageType: 'comprehensive' }, healthOfficerContext);
  ...
  describe('Special Needs - Diagnoses', () => {
    it('creates a diagnosis', async () => {
      const result = await service.createDiagnosis(tenantId, {
        studentId: 'student-001',
  ...
  describe('Special Needs - Referrals', () => {
    it('creates a referral', async () => {
      const result = await service.createReferral(tenantId, {
        studentId: 'student-001',
    ...
    it('updates referral status', async () => {
      const created = await service.createReferral(tenantId, {
        studentId: 'student-001',
        referralDate: '2024-04-15',
        referredBy: 'School Nurse',
        referredTo: 'Speech Therapist',
      ...
      const updated = await service.updateReferral(tenantId, created.id, {
        status: 'completed',
  ...
  describe('Special Needs - Accommodation Plans', () => {
    it('creates an accommodation plan', async () => {
      const result = await service.createAccommodationPlan(tenantId, {
        studentId: 'student-001',
    ...
    it('updates accommodation plan status', async () => {
      const created = await service.createAccommodationPlan(tenantId, {
        studentId: 'student-001',
      ...
      const updated = await service.updateAccommodationPlan(tenantId, created.id, {
        status: 'under-review',
      }, healthOfficerContext);
  ...
  describe('Counselling Sessions', () => {
    it('creates a counselling session', async () => {
      const result = await service.createCounsellingSession(tenantId, {
        studentId: 'student-001',
        counsellorId: 'staff-counsellor-001',
        sessionDate: '2024-03-20',
        sessionType: 'individual',
        reason: 'Anxiety about exams',
        caseNotes: 'Student expressed concerns about upcoming exams. Discussed coping strategies.',
    ...
    it('rejects session when follow-up required but no date', async () => {
      await expect(
        service.createCounsellingSession(tenantId, {
          studentId: 'student-001',
    ...
    it('updates a counselling session', async () => {
      const created = await service.createCounsellingSession(tenantId, {
        studentId: 'student-001',
        counsellorId: 'staff-001',
        sessionDate: '2024-03-20',
        sessionType: 'individual',
      ...
      const updated = await service.updateCounsellingSession(tenantId, created.id, {
        status: 'completed',
    ...
    it('lists counselling sessions by student', async () => {
      await service.createCounsellingSession(tenantId, {
        studentId: 'student-001',
        counsellorId: 'staff-001',
        sessionDate: '2024-03-20',
        sessionType: 'individual',
      ...
      const result = await service.listCounsellingSessions(tenantId, 'student-001', { page: 1, pageSize: 10 }, healthOfficerContext);
      expect(result.data).toHaveLength(2);
    });
  });
  ...
  describe('Screening Programs', () => {
    ...
    it('lists screening programs by grade level', async () => {
      ...
      const grade1 = await service.listScreeningProgramsByGrade(tenantId, 'Grade 1', { page: 1, pageSize: 10 });
    ...
    it('throws NotFoundError for non-existent program', async () => {
      await expect(
        service.getScreeningProgram(tenantId, 'non-existent-id'),
