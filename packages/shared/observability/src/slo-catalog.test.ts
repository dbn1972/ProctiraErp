...
describe('SLO Catalog', () => {
  ...
  it.each(entries)('%s has a valid service name matching its key', (key, slo: ServiceSLO) => {
    expect(slo.service).toBe(key);
  });

  it.each(entries)('%s has availability target in (0, 1]', (_key, slo: ServiceSLO) => {
  ...
  it.each(entries)('%s has p95 <= p99 latency', (_key, slo: ServiceSLO) => {
    expect(slo.indicators.latency.p95).toBeLessThanOrEqual(slo.indicators.latency.p99);
  });

  it.each(entries)('%s has positive latency targets', (_key, slo: ServiceSLO) => {
    expect(slo.indicators.latency.p95).toBeGreaterThan(0);
    expect(slo.indicators.latency.p99).toBeGreaterThan(0);
  });

  it.each(entries)('%s has error rate target in (0, 1)', (_key, slo: ServiceSLO) => {
    expect(slo.indicators.errorRate.target).toBeGreaterThan(0);
    expect(slo.indicators.errorRate.target).toBeLessThan(1);
  });

  it.each(entries)('%s has saturation targets in (0, 1]', (_key, slo: ServiceSLO) => {
  ...
  it.each(entries)('%s has a non-empty owner', (_key, slo: ServiceSLO) => {
    expect(slo.owner.trim().length).toBeGreaterThan(0);
  });

  it.each(entries)('%s has a non-empty runbook URL', (_key, slo: ServiceSLO) => {
    expect(slo.runbook.trim().length).toBeGreaterThan(0);
    expect(slo.runbook).toContain('runbooks');
  });

  it.each(entries)('%s has at least one alert defined', (_key, slo: ServiceSLO) => {
    expect(slo.alerts.length).toBeGreaterThan(0);
  });

  it.each(entries)('%s has an oncall group', (_key, slo: ServiceSLO) => {
  ...
  it('includes platform services', () => {
    const platformServices = ['workflow', 'notification', 'audit', 'report'];
