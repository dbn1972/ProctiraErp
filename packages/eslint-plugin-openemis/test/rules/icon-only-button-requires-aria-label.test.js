...
ruleTester.run('icon-only-button-requires-aria-label', rule, {
  valid: [
    ...
    {
      code: '<IconButton aria-label={`Close ${name}`}><XIcon /></IconButton>',
    },
    // aria-label via a t() call (treated as present)
    {
      code: "<IconButton aria-label={t('actions.delete')}><TrashIcon /></IconButton>",
    },
    ...
    {
      code: '<button data-icon-only aria-label="Close"><XIcon /></button>',
    },
  ...
  invalid: [
    ...
    {
      code: "import { Trash } from 'lucide-react';\n" + '<IconButton><Trash /></IconButton>',
      errors: [{ messageId: 'missingAccessibleName' }],
    },
    ...
    {
      code: "import { Trash as Bin } from 'lucide-react';\n" + '<IconButton><Bin /></IconButton>',
      errors: [{ messageId: 'missingAccessibleName' }],
    },
    ...
    {
      code: '<Button><TrashIcon /><span className="sr-only" /></Button>',
      errors: [{ messageId: 'missingAccessibleName' }],
    },
