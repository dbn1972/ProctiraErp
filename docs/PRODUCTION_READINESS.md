...
| 6.3 | Metrics export enabled (Prometheus)     | ✅                 | `GET /metrics` on api-gateway, etl-worker and the 9 standalone services via `@proctira/observability`; scraped by `infra/observability/prometheus.yml`. **No OTLP metrics exporter** — `METRICS_ENABLED=false` is the only switch                         |
| 6.4 | Distributed tracing enabled             | ✅ App-wired (W1-OPS-13) | `@proctira/observability` registers tracer provider + W3C propagation + HTTP/Fastify spans; OTLP/HTTP export when `OTEL_EXPORTER_OTLP_*` is set; **no-op when unset**. Production expects a collector URL in Helm `config.tracing` / compose env — **live collector ingest is not proven in-repo**. See `docs/audits/OPS_W1_OPS_13_OTEL.md`. |
