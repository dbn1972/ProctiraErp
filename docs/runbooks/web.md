...
| On-call group  | `web-oncall`                                  |
| Source         | `packages/backend/../apps/web` (or `apps/web` if applicable) |

## Dependencies

Upstream services and infrastructure required for `web` to operate:

- api-gateway

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `web` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="web"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="web"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=web` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/web`).
2. `kubectl rollout undo deployment/web` (or pin the image tag in the
   compose file and `docker compose up -d web`).
3. Confirm `up{service="web"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `web` repo with the offending
   commit range.
...
| ---- | -------------------------------------------------------- | ------------ |
| 1    | Primary on-call (`web-oncall` PagerDuty)              | Immediately  |
...
- Source: `packages/backend/../apps/web/`
