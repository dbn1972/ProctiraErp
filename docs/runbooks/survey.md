...
| On-call group  | `survey-oncall`                                  |
| Source         | `packages/backend/survey` (or `apps/survey` if applicable) |

## Dependencies

Upstream services and infrastructure required for `survey` to operate:

- PostgreSQL (survey DB)
- notification

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `survey` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="survey"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="survey"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=survey` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/survey`).
2. `kubectl rollout undo deployment/survey` (or pin the image tag in the
   compose file and `docker compose up -d survey`).
3. Confirm `up{service="survey"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `survey` repo with the offending
   commit range.
...
| ---- | -------------------------------------------------------- | ------------ |
| 1    | Primary on-call (`survey-oncall` PagerDuty)              | Immediately  |
...
- Source: `packages/backend/survey/`
