...
| Service slug   | `policy`                                         |
| Default port   | `3023`                                         |
| Owner team     | Security Engineering                                     |
| Slack channel  | #proctira-security                                  |
| On-call group  | `policy-oncall`                                  |
| Source         | `packages/backend/policy` (or `apps/policy` if applicable) |

## Dependencies

Upstream services and infrastructure required for `policy` to operate:

- PostgreSQL (policy DB)
- auth

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `policy` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="policy"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="policy"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=policy` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/policy`).
2. `kubectl rollout undo deployment/policy` (or pin the image tag in the
   compose file and `docker compose up -d policy`).
3. Confirm `up{service="policy"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `policy` repo with the offending
   commit range.
...
| ---- | -------------------------------------------------------- | ------------ |
| 1    | Primary on-call (`policy-oncall` PagerDuty)              | Immediately  |
| 2    | Secondary on-call                                        | +15 min      |
| 3    | Team lead, Security Engineering                                  | +30 min      |
| 4    | Engineering manager + Incident Commander                 | +60 min      |
| 5    | CTO bridge (P1 incidents only)                           | +90 min      |

Coordination happens in #proctira-security. Use `/incident declare`
...
- Source: `packages/backend/policy/`
