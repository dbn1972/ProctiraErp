...
| On-call group  | `tenant-oncall`                                  |
| Source         | `packages/backend/tenant` (or `apps/tenant` if applicable) |

## Dependencies

Upstream services and infrastructure required for `tenant` to operate:

- PostgreSQL (tenant DB)
- Redis (resolution cache)

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `tenant` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="tenant"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="tenant"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=tenant` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/tenant`).
2. `kubectl rollout undo deployment/tenant` (or pin the image tag in the
   compose file and `docker compose up -d tenant`).
3. Confirm `up{service="tenant"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `tenant` repo with the offending
   commit range.
...
| ---- | -------------------------------------------------------- | ------------ |
| 1    | Primary on-call (`tenant-oncall` PagerDuty)              | Immediately  |
...
- Source: `packages/backend/tenant/`
