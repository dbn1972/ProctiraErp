...
| On-call group  | `transport-oncall`                                  |
| Source         | `packages/backend/transport` (or `apps/transport` if applicable) |

## Dependencies

Upstream services and infrastructure required for `transport` to operate:

- PostgreSQL (transport DB)
- institution

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `transport` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="transport"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="transport"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=transport` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/transport`).
2. `kubectl rollout undo deployment/transport` (or pin the image tag in the
   compose file and `docker compose up -d transport`).
3. Confirm `up{service="transport"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `transport` repo with the offending
   commit range.
...
| ---- | -------------------------------------------------------- | ------------ |
| 1    | Primary on-call (`transport-oncall` PagerDuty)              | Immediately  |
...
- Source: `packages/backend/transport/`
