...
| On-call group | `auth-oncall`                                          |
| Source        | `packages/backend/auth` (or `apps/auth` if applicable) |

## Dependencies

Upstream services and infrastructure required for `auth` to operate:

- PostgreSQL (users, refresh tokens, sessions)
...
their dashboards before declaring `auth` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="auth"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="auth"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=auth` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/auth`).
2. `kubectl rollout undo deployment/auth` (or pin the image tag in the
   compose file and `docker compose up -d auth`).
3. Confirm `up{service="auth"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `auth` repo with the offending
   commit range.
...
| ---- | ----------------------------------------- | ------------ |
| 1    | Primary on-call (`auth-oncall` PagerDuty) | Immediately  |
