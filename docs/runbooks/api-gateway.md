...
| On-call group | `api-gateway-oncall`                                                         |
| Source        | `packages/backend/../apps/api-gateway` (or `apps/api-gateway` if applicable) |

## Dependencies

Upstream services and infrastructure required for `api-gateway` to operate:

- auth
- tenant
- every backend service downstream
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="api-gateway"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="api-gateway"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=api-gateway` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/api-gateway`).
2. `kubectl rollout undo deployment/api-gateway` (or pin the image tag in the
   compose file and `docker compose up -d api-gateway`).
3. Confirm `up{service="api-gateway"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `api-gateway` repo with the offending
   commit range.
...
| ---- | ------------------------------------------------ | ------------ |
| 1    | Primary on-call (`api-gateway-oncall` PagerDuty) | Immediately  |
...
- Source: `packages/backend/../apps/api-gateway/`
