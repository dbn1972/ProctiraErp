...
| On-call group | `audit-oncall`                                           |
| Source        | `packages/backend/audit` (or `apps/audit` if applicable) |

## Dependencies

Upstream services and infrastructure required for `audit` to operate:

- PostgreSQL (audit DB, append-only)
- Kafka (event ingest)

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `audit` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="audit"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="audit"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=audit` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/audit`).
2. `kubectl rollout undo deployment/audit` (or pin the image tag in the
   compose file and `docker compose up -d audit`).
3. Confirm `up{service="audit"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `audit` repo with the offending
   commit range.
...
| ---- | ------------------------------------------ | ------------ |
| 1    | Primary on-call (`audit-oncall` PagerDuty) | Immediately  |
