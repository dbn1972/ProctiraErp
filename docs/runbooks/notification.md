...
| Field         | Value                                                                  |
| ------------- | ---------------------------------------------------------------------- |
| Service slug  | `notification`                                                         |
| Default port  | `3012`                                                                 |
| Owner team    | Platform Engineering                                                   |
| Slack channel | #proctira-platform                                                     |
| On-call group | `notification-oncall`                                                  |
| Source        | `packages/backend/notification` (or `apps/notification` if applicable) |

## Dependencies

Upstream services and infrastructure required for `notification` to operate:

- Kafka (event ingest)
- Redis (dedupe / templating cache)
- external SMTP/SMS providers

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `notification` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="notification"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="notification"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=notification` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/notification`).
2. `kubectl rollout undo deployment/notification` (or pin the image tag in the
   compose file and `docker compose up -d notification`).
3. Confirm `up{service="notification"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `notification` repo with the offending
   commit range.
...
| ---- | ------------------------------------------------- | ------------ |
| 1    | Primary on-call (`notification-oncall` PagerDuty) | Immediately  |
...
- Source: `packages/backend/notification/`
