...
| On-call group  | `health-oncall`                                  |
| Source         | `packages/backend/health` (or `apps/health` if applicable) |

## Dependencies

Upstream services and infrastructure required for `health` to operate:
...
their dashboards before declaring `health` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="health"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="health"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=health` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/health`).
2. `kubectl rollout undo deployment/health` (or pin the image tag in the
   compose file and `docker compose up -d health`).
3. Confirm `up{service="health"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `health` repo with the offending
   commit range.
...
| ---- | -------------------------------------------------------- | ------------ |
| 1    | Primary on-call (`health-oncall` PagerDuty)              | Immediately  |
...
- Source: `packages/backend/health/`
