...
| Field         | Value                                                                |
| ------------- | -------------------------------------------------------------------- |
| Service slug  | `scholarship`                                                        |
| Default port  | `3008`                                                               |
| Owner team    | Domain Engineering                                                   |
| Slack channel | #proctira-domain                                                     |
| On-call group | `scholarship-oncall`                                                 |
| Source        | `packages/backend/scholarship` (or `apps/scholarship` if applicable) |

## Dependencies

Upstream services and infrastructure required for `scholarship` to operate:

- PostgreSQL (scholarship DB)
- student
- billing (disbursements)

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `scholarship` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="scholarship"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="scholarship"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=scholarship` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/scholarship`).
2. `kubectl rollout undo deployment/scholarship` (or pin the image tag in the
   compose file and `docker compose up -d scholarship`).
3. Confirm `up{service="scholarship"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `scholarship` repo with the offending
   commit range.
...
| ---- | ------------------------------------------------ | ------------ |
| 1    | Primary on-call (`scholarship-oncall` PagerDuty) | Immediately  |
...
- Source: `packages/backend/scholarship/`
