...
| On-call group | `report-oncall`                                            |
| Source        | `packages/backend/report` (or `apps/report` if applicable) |

## Dependencies

Upstream services and infrastructure required for `report` to operate:

- data-warehouse
- object storage (PDF / CSV exports)
- RabbitMQ

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `report` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="report"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="report"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=report` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/report`).
2. `kubectl rollout undo deployment/report` (or pin the image tag in the
   compose file and `docker compose up -d report`).
3. Confirm `up{service="report"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `report` repo with the offending
   commit range.
...
| ---- | ------------------------------------------- | ------------ |
| 1    | Primary on-call (`report-oncall` PagerDuty) | Immediately  |
...
- Source: `packages/backend/report/`
