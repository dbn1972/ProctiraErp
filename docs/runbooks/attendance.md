...
| Field          | Value                                            |
| -------------- | ------------------------------------------------ |
| Service slug   | `attendance`                                         |
| Default port   | `3006`                                         |
| Owner team     | Domain Engineering                                     |
| Slack channel  | #proctira-domain                                  |
| On-call group  | `attendance-oncall`                                  |
| Source         | `packages/backend/attendance` (or `apps/attendance` if applicable) |

## Dependencies

Upstream services and infrastructure required for `attendance` to operate:

- PostgreSQL (attendance DB)
- student
- notification (parent absence alerts)

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `attendance` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="attendance"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="attendance"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=attendance` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/attendance`).
2. `kubectl rollout undo deployment/attendance` (or pin the image tag in the
   compose file and `docker compose up -d attendance`).
3. Confirm `up{service="attendance"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `attendance` repo with the offending
   commit range.
...
| ---- | -------------------------------------------------------- | ------------ |
| 1    | Primary on-call (`attendance-oncall` PagerDuty)              | Immediately  |
...
- Source: `packages/backend/attendance/`
