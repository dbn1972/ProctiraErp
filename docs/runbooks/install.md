...
| Field         | Value                                                        |
| ------------- | ------------------------------------------------------------ |
| Service slug  | `install`                                                    |
| Default port  | `3025`                                                       |
| Owner team    | Platform Engineering                                         |
| Slack channel | #proctira-platform                                           |
| On-call group | `install-oncall`                                             |
| Source        | `packages/backend/install` (or `apps/install` if applicable) |

## Dependencies

Upstream services and infrastructure required for `install` to operate:

- PostgreSQL (admin / migrations)
- secrets store

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `install` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="install"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="install"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=install` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/install`).
2. `kubectl rollout undo deployment/install` (or pin the image tag in the
   compose file and `docker compose up -d install`).
3. Confirm `up{service="install"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `install` repo with the offending
   commit range.
...
| ---- | -------------------------------------------- | ------------ |
| 1    | Primary on-call (`install-oncall` PagerDuty) | Immediately  |
...
- Source: `packages/backend/install/`
