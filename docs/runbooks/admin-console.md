...
Upstream services and infrastructure required for `admin-console` to operate:
...
their dashboards before declaring `admin-console` itself the root cause.
...
- **Remediation**:
  1. `kubectl get pods -l app=admin-console` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/admin-console`).
2. `kubectl rollout undo deployment/admin-console` (or pin the image tag in the
   compose file and `docker compose up -d admin-console`).
3. Confirm `up{service="admin-console"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `admin-console` repo with the offending
   commit range.
...
- Source: `packages/backend/../apps/admin-console/`
