...
| Field         | Value                                                          |
| ------------- | -------------------------------------------------------------- |
| Service slug  | `workflow`                                                     |
| Default port  | `3011`                                                         |
| Owner team    | Platform Engineering                                           |
| Slack channel | #proctira-platform                                             |
| On-call group | `workflow-oncall`                                              |
| Source        | `packages/backend/workflow` (or `apps/workflow` if applicable) |

## Dependencies

Upstream services and infrastructure required for `workflow` to operate:
...
their dashboards before declaring `workflow` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="workflow"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="workflow"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=workflow` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/workflow`).
2. `kubectl rollout undo deployment/workflow` (or pin the image tag in the
   compose file and `docker compose up -d workflow`).
3. Confirm `up{service="workflow"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `workflow` repo with the offending
   commit range.
...
| ---- | --------------------------------------------- | ------------ |
| 1    | Primary on-call (`workflow-oncall` PagerDuty) | Immediately  |
...
- Source: `packages/backend/workflow/`
