...
| On-call group  | `assessment-oncall`                                  |
| Source         | `packages/backend/assessment` (or `apps/assessment` if applicable) |

## Dependencies

Upstream services and infrastructure required for `assessment` to operate:

- PostgreSQL (assessment DB)
- student (enrolments)
- report (scheduling export jobs)

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `assessment` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="assessment"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="assessment"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=assessment` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/assessment`).
2. `kubectl rollout undo deployment/assessment` (or pin the image tag in the
   compose file and `docker compose up -d assessment`).
3. Confirm `up{service="assessment"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `assessment` repo with the offending
   commit range.
...
| ---- | -------------------------------------------------------- | ------------ |
| 1    | Primary on-call (`assessment-oncall` PagerDuty)              | Immediately  |
...
- Source: `packages/backend/assessment/`
