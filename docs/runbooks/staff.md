...
| On-call group | `staff-oncall`                                           |
| Source        | `packages/backend/staff` (or `apps/staff` if applicable) |

## Dependencies

Upstream services and infrastructure required for `staff` to operate:

- PostgreSQL (staff DB)
- institution (positions, units)
- audit (write events)

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `staff` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="staff"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="staff"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=staff` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/staff`).
2. `kubectl rollout undo deployment/staff` (or pin the image tag in the
   compose file and `docker compose up -d staff`).
3. Confirm `up{service="staff"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `staff` repo with the offending
   commit range.
...
| ---- | ------------------------------------------ | ------------ |
| 1    | Primary on-call (`staff-oncall` PagerDuty) | Immediately  |
