...
| On-call group | `plugin-oncall`                                            |
| Source        | `packages/backend/plugin` (or `apps/plugin` if applicable) |

## Dependencies

Upstream services and infrastructure required for `plugin` to operate:

- PostgreSQL (plugin metadata)
- object storage (plugin bundles)
- sandbox runner

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `plugin` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="plugin"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="plugin"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=plugin` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/plugin`).
2. `kubectl rollout undo deployment/plugin` (or pin the image tag in the
   compose file and `docker compose up -d plugin`).
3. Confirm `up{service="plugin"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `plugin` repo with the offending
   commit range.
...
| ---- | ------------------------------------------- | ------------ |
| 1    | Primary on-call (`plugin-oncall` PagerDuty) | Immediately  |
...
- Source: `packages/backend/plugin/`
