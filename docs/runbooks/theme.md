...
| On-call group | `theme-oncall`                                           |
| Source        | `packages/backend/theme` (or `apps/theme` if applicable) |

## Dependencies

Upstream services and infrastructure required for `theme` to operate:

- PostgreSQL (theme DB)
- object storage (assets)

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `theme` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="theme"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="theme"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=theme` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/theme`).
2. `kubectl rollout undo deployment/theme` (or pin the image tag in the
   compose file and `docker compose up -d theme`).
3. Confirm `up{service="theme"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `theme` repo with the offending
   commit range.
...
| ---- | ------------------------------------------ | ------------ |
| 1    | Primary on-call (`theme-oncall` PagerDuty) | Immediately  |
