...
| Field         | Value                                                                                        |
| ------------- | -------------------------------------------------------------------------------------------- |
| Service slug  | `registration-portal`                                                                        |
| Default port  | `3031`                                                                                       |
| Owner team    | Frontend Engineering                                                                         |
| Slack channel | #proctira-frontend                                                                           |
| On-call group | `registration-portal-oncall`                                                                 |
| Source        | `packages/backend/../apps/registration-portal` (or `apps/registration-portal` if applicable) |

## Dependencies

Upstream services and infrastructure required for `registration-portal` to operate:

- api-gateway
- registration

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `registration-portal` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="registration-portal"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="registration-portal"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=registration-portal` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/registration-portal`).
2. `kubectl rollout undo deployment/registration-portal` (or pin the image tag in the
   compose file and `docker compose up -d registration-portal`).
3. Confirm `up{service="registration-portal"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `registration-portal` repo with the offending
   commit range.
...
| ---- | -------------------------------------------------------- | ------------ |
| 1    | Primary on-call (`registration-portal-oncall` PagerDuty) | Immediately  |
...
- Source: `packages/backend/../apps/registration-portal/`
