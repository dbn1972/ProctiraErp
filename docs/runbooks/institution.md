...
| Field         | Value                                                                |
| ------------- | -------------------------------------------------------------------- |
| Service slug  | `institution`                                                        |
| Default port  | `3002`                                                               |
| Owner team    | Domain Engineering                                                   |
| Slack channel | #proctira-domain                                                     |
| On-call group | `institution-oncall`                                                 |
| Source        | `packages/backend/institution` (or `apps/institution` if applicable) |

## Dependencies

Upstream services and infrastructure required for `institution` to operate:

- PostgreSQL (institutions DB)
- audit (write events)

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `institution` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="institution"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="institution"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=institution` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/institution`).
2. `kubectl rollout undo deployment/institution` (or pin the image tag in the
   compose file and `docker compose up -d institution`).
3. Confirm `up{service="institution"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `institution` repo with the offending
   commit range.
...
| ---- | ------------------------------------------------ | ------------ |
| 1    | Primary on-call (`institution-oncall` PagerDuty) | Immediately  |
...
- Source: `packages/backend/institution/`
