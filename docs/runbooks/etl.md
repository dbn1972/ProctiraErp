...
| On-call group  | `etl-oncall`                                  |
| Source         | `packages/backend/etl` (or `apps/etl` if applicable) |

## Dependencies

Upstream services and infrastructure required for `etl` to operate:

- PostgreSQL (operational stores, read-only)
- data-warehouse
- object storage (intermediate files)

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `etl` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="etl"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="etl"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=etl` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/etl`).
2. `kubectl rollout undo deployment/etl` (or pin the image tag in the
   compose file and `docker compose up -d etl`).
3. Confirm `up{service="etl"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `etl` repo with the offending
   commit range.
...
| ---- | -------------------------------------------------------- | ------------ |
| 1    | Primary on-call (`etl-oncall` PagerDuty)              | Immediately  |
