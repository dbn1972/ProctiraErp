...
| Field          | Value                                            |
| -------------- | ------------------------------------------------ |
| Service slug   | `student`                                         |
| Default port   | `3003`                                         |
| Owner team     | Domain Engineering                                     |
| Slack channel  | #proctira-domain                                  |
| On-call group  | `student-oncall`                                  |
| Source         | `packages/backend/student` (or `apps/student` if applicable) |

## Dependencies

Upstream services and infrastructure required for `student` to operate:

- PostgreSQL (students DB)
- institution (academic periods)
- audit (write events)

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `student` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="student"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="student"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=student` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/student`).
2. `kubectl rollout undo deployment/student` (or pin the image tag in the
   compose file and `docker compose up -d student`).
3. Confirm `up{service="student"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `student` repo with the offending
   commit range.
...
| ---- | -------------------------------------------------------- | ------------ |
| 1    | Primary on-call (`student-oncall` PagerDuty)              | Immediately  |
...
- Source: `packages/backend/student/`
