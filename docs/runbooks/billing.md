...
| On-call group  | `billing-oncall`                                  |
| Source         | `packages/backend/billing` (or `apps/billing` if applicable) |

## Dependencies

Upstream services and infrastructure required for `billing` to operate:
...
their dashboards before declaring `billing` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="billing"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="billing"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=billing` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/billing`).
2. `kubectl rollout undo deployment/billing` (or pin the image tag in the
   compose file and `docker compose up -d billing`).
3. Confirm `up{service="billing"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `billing` repo with the offending
   commit range.
...
| ---- | -------------------------------------------------------- | ------------ |
| 1    | Primary on-call (`billing-oncall` PagerDuty)              | Immediately  |
...
- Source: `packages/backend/billing/`
