...
| Field         | Value                                                                |
| ------------- | -------------------------------------------------------------------- |
| Service slug  | `examination`                                                        |
| Default port  | `3007`                                                               |
| Owner team    | Domain Engineering                                                   |
| Slack channel | #proctira-domain                                                     |
| On-call group | `examination-oncall`                                                 |
| Source        | `packages/backend/examination` (or `apps/examination` if applicable) |

## Dependencies

Upstream services and infrastructure required for `examination` to operate:

- PostgreSQL (examination DB)
- student
- report

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `examination` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="examination"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="examination"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=examination` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/examination`).
2. `kubectl rollout undo deployment/examination` (or pin the image tag in the
   compose file and `docker compose up -d examination`).
3. Confirm `up{service="examination"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `examination` repo with the offending
   commit range.
...
| ---- | ------------------------------------------------ | ------------ |
| 1    | Primary on-call (`examination-oncall` PagerDuty) | Immediately  |
...
- Source: `packages/backend/examination/`
