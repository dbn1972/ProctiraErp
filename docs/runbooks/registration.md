...
| Field          | Value                                            |
| -------------- | ------------------------------------------------ |
| Service slug   | `registration`                                         |
| Default port   | `3019`                                         |
| Owner team     | Platform Engineering                                     |
| Slack channel  | #proctira-platform                                  |
| On-call group  | `registration-oncall`                                  |
| Source         | `packages/backend/registration` (or `apps/registration` if applicable) |

## Dependencies

Upstream services and infrastructure required for `registration` to operate:

- PostgreSQL (registration DB)
- student (handover)
- notification

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `registration` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="registration"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="registration"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=registration` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/registration`).
2. `kubectl rollout undo deployment/registration` (or pin the image tag in the
   compose file and `docker compose up -d registration`).
3. Confirm `up{service="registration"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `registration` repo with the offending
   commit range.
...
| ---- | -------------------------------------------------------- | ------------ |
| 1    | Primary on-call (`registration-oncall` PagerDuty)              | Immediately  |
...
- Source: `packages/backend/registration/`
