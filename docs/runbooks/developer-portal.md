...
| Field          | Value                                            |
| -------------- | ------------------------------------------------ |
| Service slug   | `developer-portal`                                         |
| Default port   | `3026`                                         |
| Owner team     | Platform Engineering                                     |
| Slack channel  | #proctira-platform                                  |
| On-call group  | `developer-portal-oncall`                                  |
| Source         | `packages/backend/developer-portal` (or `apps/developer-portal` if applicable) |

## Dependencies

Upstream services and infrastructure required for `developer-portal` to operate:

- object storage (docs)
- auth (sandbox tokens)

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `developer-portal` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="developer-portal"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="developer-portal"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=developer-portal` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/developer-portal`).
2. `kubectl rollout undo deployment/developer-portal` (or pin the image tag in the
   compose file and `docker compose up -d developer-portal`).
3. Confirm `up{service="developer-portal"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `developer-portal` repo with the offending
   commit range.
...
| ---- | -------------------------------------------------------- | ------------ |
| 1    | Primary on-call (`developer-portal-oncall` PagerDuty)              | Immediately  |
| 2    | Secondary on-call                                        | +15 min      |
| 3    | Team lead, Platform Engineering                                  | +30 min      |
| 4    | Engineering manager + Incident Commander                 | +60 min      |
| 5    | CTO bridge (P1 incidents only)                           | +90 min      |

Coordination happens in #proctira-platform. Use `/incident declare`
...
- Source: `packages/backend/developer-portal/`
