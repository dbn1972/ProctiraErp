...
| Field         | Value                                                                      |
| ------------- | -------------------------------------------------------------------------- |
| Service slug  | `data-warehouse`                                                           |
| Default port  | `3018`                                                                     |
| Owner team    | Data Platform                                                              |
| Slack channel | #proctira-data                                                             |
| On-call group | `data-warehouse-oncall`                                                    |
| Source        | `packages/backend/data-warehouse` (or `apps/data-warehouse` if applicable) |

## Dependencies

Upstream services and infrastructure required for `data-warehouse` to operate:

- columnar store (e.g. ClickHouse / Postgres)
- object storage

If any of the above are unhealthy, expect cascading failures here. Check
their dashboards before declaring `data-warehouse` itself the root cause.
...
- **Remediation**:
  ...
  3. Inspect logs in Loki / pino with `service="data-warehouse"`.
...
- **Symptom**: `ServiceDown` firing; `up{service="data-warehouse"}` is 0.
- **Likely cause**: process crashed, network partition, or container OOM.
- **Remediation**:
  1. `kubectl get pods -l app=data-warehouse` (or `docker compose ps`).
...
1. Identify the last known good image tag from the deployment history
   (Argo CD / `kubectl rollout history deployment/data-warehouse`).
2. `kubectl rollout undo deployment/data-warehouse` (or pin the image tag in the
   compose file and `docker compose up -d data-warehouse`).
3. Confirm `up{service="data-warehouse"} == 1` and that error rate has dropped
   in Grafana.
4. Open a post-incident ticket in the `data-warehouse` repo with the offending
   commit range.
...
| ---- | --------------------------------------------------- | ------------ |
| 1    | Primary on-call (`data-warehouse-oncall` PagerDuty) | Immediately  |
| 2    | Secondary on-call                                   | +15 min      |
| 3    | Team lead, Data Platform                            | +30 min      |
| 4    | Engineering manager + Incident Commander            | +60 min      |
| 5    | CTO bridge (P1 incidents only)                      | +90 min      |

Coordination happens in #proctira-data. Use `/incident declare`
...
- Source: `packages/backend/data-warehouse/`
