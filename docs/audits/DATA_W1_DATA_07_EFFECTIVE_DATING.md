...
- Prisma `AcademicPeriod` model does not expose `valid_*` columns (trigger keeps them mirrored from start/end).
...
Closed with residuals documented above (no production evidence invented).
