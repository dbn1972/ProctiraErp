...
| RLS enabled means isolation enforced                  | `PgDocumentCollection` binds `app.platform_admin='1'` on every call (`pg-document-store.ts:40`) and the `control_plane_documents` policy accepts that as a full escape. Re-verified live 2026-09-20 as `proctira_app`: tenant B read tenant A's document, and **21 rows across 13 tenants** were visible under the escape. Affects `auth` (identities, users, otp, invites) and `billing` (plans, subscriptions, entitlements, usage). Tracked as P0 in `docs/audits/SEC_CONTROL_PLANE_DOCUMENT_ISOLATION.md`. Check the policy predicate, not `relrowsecurity` |
| A green W1-DATA-06 means every tenant table has an FK | `check-strict-tenant-fks.mjs:77` still filters `data_type='uuid'`, but #322 added a second CTE that reports `text` tenant_id columns as `text_tenant_no_fk` against an allowlist. Re-verified 2026-09-20: the gate passes **while disclosing** "23 allowlisted text tenant_id table(s) still lack a tenants(id) FK (RLS-enforced but no referential integrity)". Read the notes, not just the status                                                                                                                                                            |
