...
| A gate reporting **pass**        | what the gate actually queries               | W1-DATA-06 once skipped 23 tables because it filtered `data_type='uuid'`. Closed by #322: a second CTE now reports them as `text_tenant_no_fk`. Re-verified 2026-09-20 — the gate passes **and** discloses "23 allowlisted text tenant_id table(s) still lack a tenants(id) FK". The gate's blindness is fixed; the 23 missing FKs remain open |
...
| Statutory returns         | Does **any** government return format exist? Re-verified 2026-09-20 (word-boundary): **zero** references to APAAR, NDEAR, CEDS, OneRoster, Ed-Fi, SIF, LTI. **Not** zero for UDISE (10) and DIKSHA (1) — but every hit is marketing copy or a UI label, never an implementation: `public-website` claims "UDISE+ ready" and `{ label: 'DIKSHA content' }`, and `apps/web` has a `FactRow label="UDISE code"` plus an **"Export UDISE" button with no `onClick`, no `asChild`, no handler**. `packages/backend` and `apps/api-gateway/src` contain **zero** UDISE references. Treat an affirmative public capability claim with a dead control behind it as worse than absence. `board_export_jobs` exists but is referenced only by `gradebook` |
