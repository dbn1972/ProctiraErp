...
Tip CI on `de75315`+: **pending** at the time of writing — the `ci` pillar flips only when the run on the fixed tip is green (subscription active on the branch).
...
| Batch-3 gaps (G-909, G-915–G-922) not yet merged                  | This document covers the merged slice only                                                                   | Wave 9 integration | Branches pushed (`cursor/w9-g909…`, `g915…`, `g916…`, `g917…`, `g918…`, `g920…`); evidence rows to be appended per merge |
