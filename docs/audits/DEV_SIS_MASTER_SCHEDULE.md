...
| Roles (RBAC) | Registrar / Scheduler: write + publish; Teacher: read; Principal: read — **gateway tenant middleware only**; live role-deny residual |
...
| RBAC enforced | ☐ | Residual — tenant-only; publish role-deny not live-tested |
...
| Audit trail for schedule publish | ☐ | Residual — status + published_at on row only |
