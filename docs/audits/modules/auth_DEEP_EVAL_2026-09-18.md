...
> **Scope note.** The document-store findings below are platform-level, not
> specific to `auth`. `PgDocumentCollection` backs 14 collections across `auth`
> (5), `tenant` (6) and `billing` (3), plus the gateway. Remediation belongs in
> `packages/shared/database/src/pg-document-store.ts` and
> `tools/scripts/check-strict-tenant-fks.mjs` once, not per module.
...
### 4. Security — `partial`, **P1** (RLS backstop) and **P0** (gate coverage)
...
**This is missing defence-in-depth, not active exposure.** An earlier draft of
this report called it P0 and said there was "no backstop" without crediting the
application-level controls. Corrected: every call site reviewed does scope
correctly, by one of three means.

| Control                       | Where                                                                                                           |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------- |
| High-entropy bearer token     | `pg-otp-store.ts:34` — knowing the MFA token is the authorisation                                               |
| Tenant-prefixed composite key | `users.get(\`${tenantId}:${email}\`)`; roles `${tenantId}::${roleId}`; entitlements `${tenantId}:${featureKey}` |
| Post-fetch ownership check    | `findRoleById` returns null unless `doc.tenantId === tenantId`; `deleteRole` and `updateRole` gate on it        |
| Correctly scoped query        | invites via `byTenant`                                                                                          |

So isolation holds today. What is absent is any enforcement below the
application layer: a new call site that omits the ownership check, or any use of
`all()` on a tenant-owned collection, returns cross-tenant records and neither
RLS nor the FK gate would catch it. Severity is therefore **P1** — a latent
regression surface, not a present leak.

One path still worth changing regardless:
`billing/pg-billing-repository.ts:204` `deleteEntitlementsBySubscription(subscriptionId)`
accepts no tenant, queries `where({ subscriptionId })` across all tenants, then
deletes each row using that row's own `tenantId`. Self-consistent, but there is
no ownership assertion anywhere in the path, so a reused or attacker-supplied
`subscriptionId` would delete another tenant's entitlements.
...
| -------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- | --------------------------- | ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- | ------ | ---------- |
| Tenant isolation backstop for document-store state | Probe 3: platform_admin escape returns cross-tenant row; `pg-document-store.ts:40`. Application controls verified present at every call site reviewed | partial         | no                          | RLS enforces isolation independent of application code                   | No enforcement below the application layer; `get`/`all`/`delete` have no tenant predicate | Latent: a future call site omitting the ownership check leaks with nothing to catch it. Not a present leak | Add tenant-scoped read methods; stop binding platform_admin for tenant-owned collections; reserve the escape for provisioning and platform collections | **P1**   | M      | High       |
| Unscoped entitlement deletion                      | `billing/pg-billing-repository.ts:204` `deleteEntitlementsBySubscription` takes no tenant and queries across all tenants                              | partial         | no                          | Deletion asserts tenant ownership                                        | No ownership assertion anywhere in the path                                               | A reused or supplied subscriptionId deletes another tenant's entitlements                                  | Require tenantId and scope the query                                                                                                                   | **P1**   | S      | High       |
...
**P1-0 — RLS is the isolation mechanism.** A live test asserts that, as
