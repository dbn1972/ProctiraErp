...
(function () {
  ...
  const icons = {
    ...
    barchart: I('<line x1="12" y1="20" x2="12" y2="10"/><line x1="18" y1="20" x2="18" y2="4"/><line x1="6" y1="20" x2="6" y2="16"/>'),
    ...
    trendup: I('<polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/>'),
    ...
    school: I('<path d="m4 6 8-4 8 4"/><path d="m18 10 4 2v8a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2v-8l4-2"/><path d="M14 22v-4a2 2 0 0 0-4 0v4"/><path d="M18 5v17M6 5v17"/><circle cx="12" cy="9" r="2"/>'),
    ...
    alert: I('<path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/>'),
    ...
    mapview: I('<polygon points="3 6 9 3 15 6 21 3 21 18 15 21 9 18 3 21"/><line x1="9" y1="3" x2="9" y2="18"/><line x1="15" y1="6" x2="15" y2="21"/>'),
  ...
  const NAV = [
    ...
    { label: 'Academics', items: [
      { id: 'institutions', icon: 'school', text: 'Institutions', href: 'institutions-list.html' },
      { id: 'academic-periods', icon: 'calendar', text: 'Academic Periods', href: 'academic-periods-list.html' },
      { id: 'attendance', icon: 'checkcircle', text: 'Attendance', href: 'attendance-mark.html' },
      { id: 'assessments', icon: 'clipboard', text: 'Assessments', href: 'assessments-list.html' },
      { id: 'examinations', icon: 'filetext', text: 'Examinations', href: 'examinations-list.html' },
    ]},
  ...
  bn.innerHTML = BOTTOM_NAV.map(it =>
    `<a href="${it.href}" class="${it.id === active ? 'active' : ''}">${icons[it.icon]}<span>${it.text}</span></a>`).join('');
  ...
  bn.querySelector('a[href="#menu"]')?.addEventListener('click', (e) => { e.preventDefault(); openDrawer(); });
  ...
  cmdk.innerHTML = `
    <div class="cmdk" role="dialog" aria-label="Command palette">
      ...
      <div class="cmdk-list">
        ...
        ${NAV.flatMap(g => g.items).map(it => `<div class="cmdk-item" data-href="${it.href}">${icons[it.icon]} ${it.text}</div>`).join('')}
      </div>
    </div>`;
  document.body.appendChild(cmdk);
  const openCmdk = () => { cmdk.classList.add('open'); cmdk.querySelector('input').focus(); };
