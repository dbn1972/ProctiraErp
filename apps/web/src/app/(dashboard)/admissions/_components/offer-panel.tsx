      ...
      <CardContent className="space-y-4">
        ...
        >
          <FormField id="offer-fee" label="Offer fee">
            <Input id="offer-fee" name="feeAmount" type="number" min={0} defaultValue={0} data-testid="offer-fee" disabled={!hydrated || pending} />
          </FormField>
          <Button type="submit" data-testid="create-offer" disabled={!hydrated || pending || !bundle.placement}>
            Draft offer
          </Button>
        </form>
        ...
        {offers.length === 0 ? (
          <p className="text-sm text-muted-foreground" role="status">
            {bundle.placement ? 'No offers yet.' : 'Set grade/period placement before creating an offer.'}
          </p>
        ) : (
...
function OfferRow({
  ...
  return (
    <li className="rounded-md border border-border p-3" data-testid="offer-row" data-status={offer.status}>
      <p className="text-sm font-medium">
        {offer.status} · {offer.feeAmount} {offer.feeCurrency}
      </p>
      {offer.paymentRef ? <p className="text-xs text-muted-foreground">Ref {offer.paymentRef}</p> : null}
      <div className="mt-2 flex flex-wrap gap-2">
        {offer.status === 'draft' ? (
          <Button type="button" size="sm" data-testid="send-offer" disabled={!hydrated || pending} onClick={onSend}>
            Send
          </Button>
        ) : null}
        {offer.status === 'sent' || offer.status === 'draft' ? (
          ...
          >
            ...
            <Button type="submit" size="sm" data-testid="accept-offer" disabled={!hydrated || pending}>
              Accept
            </Button>
          </form>
        ) : null}
        {offer.status !== 'accepted' && offer.status !== 'declined' ? (
          <Button type="button" size="sm" variant="outline" data-testid="decline-offer" disabled={!hydrated || pending} onClick={onDecline}>
            Decline
          </Button>
        ) : null}
      </div>
