import { listLibraryTitles } from '@/lib/api/library';

export const dynamic = 'force-dynamic';

export default async function LibraryPage() {
  const rows = await listLibraryTitles();
  return (
    <section className="space-y-6" aria-labelledby="library-heading">
      <div>
        <h1 id="library-heading" className="text-3xl font-extrabold tracking-tight">
          Library
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Phase 20 MVP — LibraryTitle list
        </p>
      </div>
