...
import { validateDatabaseConfig, type DatabaseFieldErrors } from '@/lib/database-validation';
      ...
      setResult({
        ...
        error:
          err instanceof Error
            ? err.message
            : 'Unable to reach the install service. Ensure the backend is running.',
      });
    } finally {
      setTesting(false);
    }
    ...
    >
      <div className="space-y-4" data-testid="database-step">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label htmlFor="db-host" className="label">
              Host
            </label>
            <input
              id="db-host"
            ...
            />
            {fieldErrors.host ? (
              <p className="error-text" data-testid="db-error-host">
                {fieldErrors.host}
              </p>
            ) : null}
          </div>
          <div>
            ...
            />
            {fieldErrors.port ? (
              <p className="error-text" data-testid="db-error-port">
                {fieldErrors.port}
              </p>
            ) : null}
          </div>
          <div>
            <label htmlFor="db-name" className="label">
              Database Name
            </label>
            <input
              id="db-name"
            ...
            {fieldErrors.database ? (
              <p className="error-text" data-testid="db-error-database">
                {fieldErrors.database}
              </p>
            ) : null}
          </div>
          <div>
            <label htmlFor="db-pool" className="label">
              Pool Size
            </label>
            <input
              id="db-pool"
              ...
              onChange={(e) =>
                setConfig((prev) => ({ ...prev, poolSize: parseInt(e.target.value, 10) || 10 }))
              }
              min={1}
              max={100}
            />
            {fieldErrors.poolSize ? (
              <p className="error-text" data-testid="db-error-pool">
                {fieldErrors.poolSize}
              </p>
            ) : null}
          </div>
          <div>
            <label htmlFor="db-user" className="label">
              Username
            </label>
            <input
              id="db-user"
            ...
            {fieldErrors.username ? (
              <p className="error-text" data-testid="db-error-username">
                {fieldErrors.username}
              </p>
            ) : null}
          </div>
          <div>
            <label htmlFor="db-pass" className="label">
              Password
            </label>
            <input
              id="db-pass"
            ...
            {fieldErrors.password ? (
              <p className="error-text" data-testid="db-error-password">
                {fieldErrors.password}
              </p>
            ) : null}
          </div>
