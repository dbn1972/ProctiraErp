/**
 * Multidevice screen capture for Registration Portal.
 *
 *   node scripts/capture-multidevice.mjs [desktop|tablet|mobile ...]
 *
 * Writes PNGs under /opt/cursor/artifacts/registration-portal-audit/multidevice/
 * Requires the app at PLAYWRIGHT_BASE_URL (default :3002).
 */
...
const OUT_ROOT =
  ...
  '/opt/cursor/artifacts/registration-portal-audit/multidevice';
...
const ROUTES = [
  ...
  ['apply-documents', '/apply/primary/documents'],
  ['apply-review', '/apply/primary/review'],
...
await writeFile(
  ...
  JSON.stringify(
    {
      pack: 'registration-portal-audit/multidevice',
      baseUrl: BASE_URL,
      viewports: ACTIVE,
      count: files.length,
      files: files.sort(),
